...
migrations.forEach((m, i) =>
    db.run(`INSERT INTO migrations (migration, batch) VALUES (?, 1)`, [m]),
);
...
prodiNames.forEach((nama) =>
    db.run(
        `INSERT INTO prodis (nama, created_at, updated_at) VALUES (?, ?, ?)`,
        [nama, now, now],
    ),
);
const prodis = {};
db.exec("SELECT id, nama FROM prodis").forEach((r) =>
    r.values.forEach(([id, nama]) => (prodis[nama] = id)),
);
...
kelasNames.forEach((nama) =>
    db.run(
        `INSERT INTO kelas (nama, created_at, updated_at) VALUES (?, ?, ?)`,
        [nama, now, now],
    ),
);
const kelas = {};
db.exec("SELECT id, nama FROM kelas").forEach((r) =>
    r.values.forEach(([id, nama]) => (kelas[nama] = id)),
);

// Mata Kuliah
const mkData = [
    ["Pemrograman Web", "IF101"],
    ["Basis Data", "IF102"],
    ["Jaringan Komputer", "IF103"],
    ["Sistem Operasi", "IF104"],
    ["Algoritma & Pemrograman", "IF105"],
    ["Rekayasa Perangkat Lunak", "IF106"],
];
mkData.forEach(([nama, kode]) =>
    db.run(
...
const mks = db
    .exec("SELECT id FROM mata_kuliahs ORDER BY id")
    .map((r) => r.values.map((v) => v[0]))[0];
...
jadwalData.forEach(([mkId, kId, hari, mulai, selesai]) =>
    db.run(
...
const mahasiswaData = [
    {
        name: "Neilsya Putri",
        email: "neilsyaputri@student.telkom.ac.id",
        nim: "103042310048",
        prodi: "Informatika",
        kel: "IF-02",
    },
    {
        name: "Rivaldo Tandoko",
        email: "rivaldo@student.telkom.ac.id",
        nim: "1034217006",
        prodi: "Informatika",
        kel: "IF-02",
    },
    {
        name: "Wahyu Pratama",
        email: "wahyupratama@student.telkom.ac.id",
        nim: "1034217008",
        prodi: "Informatika",
        kel: "IF-02",
    },
    {
        name: "Aiqbal Hermawan",
        email: "aiqbal@student.telkom.ac.id",
        nim: "1034219030",
        prodi: "Informatika",
        kel: "IF-02",
    },
    ...
    },
    {
        ...
        email: "farhan@student.telkom.ac.id",
        nim: "1034221008",
        prodi: "Informatika",
        kel: "IF-01",
    },
...
mahasiswaData.forEach(({ name, email, nim, prodi, kel }) => {
    ...
    db.run(
        `INSERT INTO mahasiswas (user_id, nim, no_hp, prodi_id, kelas_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
for (let i = 5; i >= 1; i--) {
    ...
    db.run(
        `INSERT INTO sesis (mata_kuliah_id, kelas_id, user_id, token, qr_data, durasi, status, started_at, ended_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            mk,
            kelas["IF-02"],
            adminId,
    ...
    mahasiswaIds.slice(0, 4).forEach((mhsId, j) => {
        ...
        db.run(
            `INSERT INTO absensis (sesi_id, mahasiswa_id, status, waktu_scan, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`,
            [
                sesiId,
                mhsId,
                status,
                `${d.toISOString().substring(0, 10)} 08:0${j}:00`,
                dateStr,
                dateStr,
            ],
        );
...
console.log(
    `✅ SQLite database created at ${dbPath} (${(data.length / 1024).toFixed(1)} KB)`,
);
db.close();
